...
def upload_data(data_n_clicks, # Tracks numbers of clicks of the upload button 
                contents,      # Tracks selected file
                button3_state  # Upload button state
                ):
    cty = callback_context
    print('Triggering input cb1 :', cty.triggered[0]['prop_id'])
    ...
    if data_n_clicks > button3_state and contents is not None:
        ...
        try:
            ...
            # Set dtype of Flag column to be a 16 bit float as no more is needed
            df['Flag'] = df['Flag'].astype(np.float16)
        ...
        return df.to_dict('records'), button3_state
    
    else:
        return None, button3_state
...
def update_plot(stored_data,     # Full DataFrame
    ...
    print('Triggering input cb2 :', ctx.triggered[0]['prop_id'])

    if stored_data is not None:
        df = pd.DataFrame(stored_data)
        if figure['data'][0]['customdata'][0][0] is None:
    ...
    elif (last_x_range is not None and relayout_data is not None) and \
        next((False for axrng in relayout_data if 'xaxis.autorange' in axrng), True):
    ...
    elif (last_y_range is not None and relayout_data is not None) and \
        next((False for axrng in relayout_data if 'yaxis.autorange' in axrng), True):
    ...
    if n_clicks > button1_state and selected_indices and new_value is not None:
        ...
        return figure, button1_state, button2_state, df_filtered_top100[['x', 'y', 'Flag']].to_dict('records'), None, \
               df_filtered_top100.to_dict('records'), last_x_range, last_y_range
    ...
    return figure, button1_state, button2_state, df_filtered_top100[['x', 'y', 'Flag']].to_dict('records'), selectedData, \
           df_filtered_top100.to_dict('records'), last_x_range, last_y_range
